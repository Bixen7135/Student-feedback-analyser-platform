...
export default function AnalysisResultsPage() {
  ...
  }, [analysisId, page, sortCol, sortOrder, filters, search, columns.length]);
                ...
                >
                  <div>
                    ...
                    <span style={{ fontSize: "11px", color: "var(--text-tertiary)", marginLeft: "8px" }}>
                      ...
                      {sf.filter_config.search ? `, search: "${sf.filter_config.search}"` : ""}
                    </span>
                  </div>
        ...
        >
          ...
          <div className="flex gap-3 items-center" style={{ marginBottom: "12px" }}>
            ...
            <input
              ...
              onChange={(e) => setAnomalyThreshold(parseFloat(e.target.value) || 0.6)}
          ...
          {anomalyLoading ? (
            <p style={{ fontSize: "12px", color: "var(--text-tertiary)" }}>Scanning for anomalies…</p>
          ) : anomalies !== null ? (
            anomalies.length === 0 ? (
              <p style={{ fontSize: "12px", color: "var(--text-tertiary)" }}>
              ...
              <div>
                ...
                <div style={{ maxHeight: "260px", overflowY: "auto", borderRadius: "6px", border: "1px solid var(--border-dim)" }}>
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "11px" }}>
                    <thead>
                      <tr style={{ background: "var(--bg-base)" }}>
                        <th style={{ padding: "7px 10px", textAlign: "left", color: "var(--text-tertiary)", borderBottom: "1px solid var(--border-dim)", whiteSpace: "nowrap" }}>Row #</th>
                        <th style={{ padding: "7px 10px", textAlign: "left", color: "var(--text-tertiary)", borderBottom: "1px solid var(--border-dim)" }}>Reasons</th>
                        {confCols.slice(0, 3).map((cc) => (
                          <th key={cc} style={{ padding: "7px 10px", textAlign: "left", color: "var(--gold)", borderBottom: "1px solid var(--border-dim)", whiteSpace: "nowrap" }}>{cc}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {anomalies.map((a) => (
                        <tr key={a.row_index} style={{ borderBottom: "1px solid var(--border-dim)" }}>
                          <td style={{ padding: "6px 10px", color: "var(--text-tertiary)", fontFamily: "var(--font-jetbrains)" }}>{a.row_index}</td>
                          <td style={{ padding: "6px 10px", color: "var(--text-secondary)" }}>{a.reasons.join("; ")}</td>
                          {confCols.slice(0, 3).map((cc) => (
                            <td key={cc} style={{ padding: "6px 10px", color: "var(--error, #ef4444)", fontFamily: "var(--font-jetbrains)" }}>
                              {a.data[cc] ?? "—"}
                            </td>
                          ))}
                        </tr>
      ...
      >
        <div className="flex items-center justify-between" style={{ marginBottom: "10px" }}>
          <span style={{ fontSize: "12px", fontWeight: 600, color: "var(--text-secondary)" }}>
            Filters {filters.length > 0 ? `(${filters.length} rule${filters.length > 1 ? "s" : ""})` : ""}
          </span>
          <div className="flex gap-2">
            <button
            ...
            {filters.length > 0 && (
              <>
                <button onClick={handleApplyFilters} style={{ ...btnGold, padding: "4px 12px", fontSize: "11px" }}>
                  Apply
                </button>
                <button
            ...
            {filters.map((f, idx) => (
              <div key={idx} className="flex gap-2 items-center">
                ...
                <button
                  ...
                  style={{ ...btnOutline, padding: "5px 8px", fontSize: "11px", color: "var(--text-tertiary)" }}
                >
                  ✕
                </button>
              </div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "12px" }}>
            ...
            <tbody>
              {results.rows.map((row, ri) => (
                <tr
                  key={ri}
                  style={{ borderBottom: "1px solid var(--border-dim)", opacity: loading ? 0.5 : 1 }}
                >
                  {row.map((cell, ci) => {
                    ...
                    const isLowConf = isConf && parseFloat(cell) < anomalyThreshold && !isNaN(parseFloat(cell));
                    return (
                      <td
                        key={ci}
                        style={{
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between" style={{ marginTop: "16px" }}>
          <span style={{ fontSize: "12px", color: "var(--text-tertiary)" }}>
            ...
            {results?.total_rows.toLocaleString()} rows ·{" "}
            showing {page * PAGE_SIZE + 1}–{Math.min((page + 1) * PAGE_SIZE, results?.total_rows ?? 0)}
          </span>
          <div className="flex gap-2">
            <button
